var InstallMetrics = {
  ...
  running: false,
  init: function() { 
      $("#calculate-button").click(function(ev) { InstallMetrics.calculateNow(); });
  ...
  fetchSummaryNow: function() {
      ...
      this.fetchSummaryLoop(null, {total_users: 0, total_from_ads: 0, total_from_newsfeeds: 0, total_from_unknown: 0} , function(data) {
          ...
          $('#total-from-ads').text(data.total_from_ads);
          $('#total-from-newsfeeds').text(data.total_from_newsfeeds);
          $('#total-from-unknown').text(data.total_from_unknown);
      });
  },
